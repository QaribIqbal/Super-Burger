...
/**
 * @param {number} frameCount
 * @param {number} criticalFrameCount
 */
export function createCriticalFrameOrder(frameCount, criticalFrameCount) {
  const count = Math.min(Math.max(0, criticalFrameCount), Math.max(0, frameCount));
 ...
 * @param {number} criticalFrameCount
 */
export function hasCriticalFrameSetReady(statuses, criticalFrameCount) {
  return statuses
    .slice(0, Math.max(0, criticalFrameCount))
